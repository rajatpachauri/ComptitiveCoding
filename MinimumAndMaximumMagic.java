...
public class Solution {
    public int[] solve(int[] A) {
        Arrays.sort(A);
        int mod = 1000000007;
        Long first=0L, second=0L, third=0L, fourth=0L;
        ...
        int[] res = {(int)(((Math.abs(fourth-third)%mod)+mod)%mod), (int)(((Math.abs(second-first)%mod)+mod)%mod)};
        if(Math.abs(second-first)>Math.abs(fourth-third)){
            int temp = res[1];
            res[1] = res[0];
            res[0] = temp;
        }
