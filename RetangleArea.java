...
public class Solution {
    public int solve(int A, int B, int C, int D, int E, int F, int G, int H) {
        if(H<=B || D<=F || G<=A || C<=E)
            return 0;
        if(A<=E){
            return Math.abs((Integer.min(C, G)-E)*(Integer.max(B,F)-Integer.min(D, H)));
        }
        if(G<=C){
            return Math.abs((Integer.max(A,E)-G)*(Integer.max(B,F)-Integer.min(D, H)));
        }
        return Math.abs((C-A)*(Integer.max(B,F)-Integer.min(D, H)));
    }
}
